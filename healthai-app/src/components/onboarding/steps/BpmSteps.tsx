// Flux de mesure du pouls : intro, décompte, anneau 30 s, saisie des battements.
          ...
          {steps.map((s, i) => (
            <View key={i} style={styles.stepRow}>
              <View style={styles.stepNo}>
                <Text style={styles.stepNoTx}>{i + 1}</Text>
              </View>
...
/* ---------------- Décompte 3 · 2 · 1 ---------------- */
export function BpmCountdown({ onDone }: { onDone: () => void }) {
...
/* ---------------- Anneau de mesure 30 s ---------------- */
export function BpmMeasure({ duration = 30000, onDone }: { duration?: number; onDone: () => void }) {
  const R = 132;
  const C = 2 * Math.PI * R;
  ...
  useEffect(() => {
    ...
    loop.start();
    Animated.timing(progress, { toValue: 1, duration, easing: Easing.linear, useNativeDriver: false }).start(
      ({ finished: done }) => {
    ...
    <View>
      ...
      <View style={styles.countStep}>
        <View style={styles.stepper}>
          <Pressable
            style={({ pressed }) => [styles.stepBtn, pressed && styles.stepBtnActive]}
            onPress={() => adjust(-1)}>
...
const styles = StyleSheet.create({
  ...
  stepNoTx: { fontWeight: '600', fontSize: 13, color: accent },
  stepText: { flex: 1, fontSize: 14.5, color: colors.tx2, lineHeight: 21 },

  /* takeover */
  takeover: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.bg, paddingHorizontal: 40 },
  cdLabel: { fontWeight: '600', letterSpacing: 2.6, fontSize: 13, color: colors.tx3 },
  cdNum: { fontWeight: '600', fontSize: 150, color: accent, lineHeight: 168, marginVertical: 18, ...tabular },
  cdHint: { fontSize: 14, color: colors.tx2 },
  ...
  measureNum: { fontWeight: '600', fontSize: 66, color: colors.tx, lineHeight: 70, ...tabular },
  measureUnit: { fontSize: 14, color: colors.tx2 },
  measureHint: { marginTop: 44, fontSize: 15, color: colors.tx2 },
  ...
  stepBtnTx: { fontSize: 30, color: colors.tx, lineHeight: 34 },
  stepVal: { alignItems: 'center', minWidth: 132 },
  stepNum: { fontWeight: '600', fontSize: 74, color: colors.tx, lineHeight: 78, ...tabular },
  stepCap: { fontSize: 12, color: colors.tx3, letterSpacing: 1, marginTop: 8 },
  resultPill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 9,
  ...
  resultNum: { fontWeight: '600', fontSize: 20, color: accent, ...tabular },
  resultUnit: { fontSize: 13, color: colors.tx2 },
  note: { fontSize: 13, color: colors.tx3, textAlign: 'center', lineHeight: 20, maxWidth: 290 },
});
