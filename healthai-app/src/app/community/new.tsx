...
export default function NewPostScreen() {
  ...
  const submit = async () => {
    if (!user?.id || (!text.trim() && !image)) return;
    setSubmitting(true);
    setError('');
    try {
  ...
  const canSubmit = !!user?.id && (text.trim().length > 0 || !!image) && !submitting;

  return (
    <ThemedView style={styles.root}>
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right', 'bottom']}>
        <ScreenHeader title="Nouveau post" />
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <Button label="Publier" onPress={submit} loading={submitting} disabled={!canSubmit} />
        </ScrollView>
      </SafeAreaView>
    </ThemedView>
  );
