...
const toStr = (val: number | null | undefined) =>
    val !== null && val !== undefined ? String(val) : "";

export default function UpdateFoodModal({ food, onClose, onSuccess }: Props) {
    const [form, setForm] = useState({
        name: food.name ?? "",
        category_id: food.category_id ?? "",
        calories_kcal: toStr(food.calories_kcal),
        proteins_g: toStr(food.proteins_g),
        carbs_g: toStr(food.carbs_g),
        fats_g: toStr(food.fats_g),
        fiber_g: toStr(food.fiber_g),
        sugars_g: toStr(food.sugars_g),
        sodium_mg: toStr(food.sodium_mg),
        cholesterol_mg: toStr(food.cholesterol_mg),
        meal_type: food.meal_type ?? "",
        water_intake_ml: toStr(food.water_intake_ml),
        source: food.source ?? "",
    });
        ...
        >
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    {/* Apport en eau + Source */}
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-sm text-gray-400">Apport en eau (ml)</label>
                            <input
                                type="number"
                    ...
                    <div className="flex justify-end gap-3 mt-2">
                        <button
                            type="button"
                            onClick={onClose}
                            className="px-4 py-2 text-sm rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-300 transition-colors"
                        >
                            Annuler
                        </button>
                        <button
                            type="submit"
                            disabled={loading}
                            className="px-4 py-2 text-sm rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-semibold transition-colors disabled:opacity-50"
                        >
