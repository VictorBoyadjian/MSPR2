...
DUMP_FILE="$(dirname "$0")/pg_restore/dump.sql"
DB_USER=postgres
DB_NAME=healthai
...
# Copier le dump dans le conteneur et le rejouer avec psql (dump SQL brut)
docker cp "$DUMP_FILE" postgres:/tmp/dump.sql

echo "[INFO] Lancement du restore..."
docker exec -e PGPASSWORD=postgres postgres \
    psql -U "$DB_USER" -d "$DB_NAME" -f /tmp/dump.sql || true
